...
KLineRevision Revision(std::uint64_t version,
                       std::uint64_t bucket_start_seconds,
                       RevisionOperation operation,
                       std::int64_t volume,
                       Identifier128 recovery_run) {
    KLineRevision revision{};
    const std::uint64_t bucket_start =
        bucket_start_seconds * kNanosecondsPerSecond;
    revision.key = KLineKey{20260807U, Market::kShenzhen, 1U, 1U,
                            bucket_start};
    ...
    const std::int64_t price = 10'000'000 + volume;
    revision.payload.bucket_end_ns_from_midnight =
        bucket_start + kNanosecondsPerSecond;
    ...
    revision.payload.volume = volume;
    revision.payload.notional_p6 = price * volume;
    revision.payload.trade_count = 1U;
    revision.payload.first_trade = TradeAnchor{
        bucket_start + 1U, 0U, bucket_start_seconds + 1U,
        bucket_start_seconds + 1'000U};
...
KLineClickHouseConfig HttpConfig(const RetryHttpServer& server) {
    KLineClickHouseConfig config{};
...
void TestUnknownOutcomeRetriesExactRevisionChunkBeforeCommit() {
    ...
    if (!server.valid()) {
        std::cout << "KLine exact-retry fixture skipped; loopback sockets "
                     "are unavailable\n";
        return;
    }
...
void TestOrderedWriterLanesKeepOwnerAffinity() {
    RetryHttpServer server(std::chrono::milliseconds{0}, false);
    if (!server.valid()) {
        std::cout << "KLine writer-lane fixture skipped; loopback sockets "
                     "are unavailable\n";
        return;
    }
    KLineClickHouseConfig config = HttpConfig(server);
...
void TestClickHouseIntegration(std::string endpoint) {
    ...
    const std::string database =
        "l2kline_integration_" + std::to_string(unique);
    KLineClickHouseConfig config{};
    ...
    KLineRevision tombstone = Revision(
    ...
    CHECK(Query(config.endpoint,
                "SELECT count() FROM " + database +
                    ".kline_revision_log FORMAT TSV") == "4");
    CHECK(Query(config.endpoint,
                "SELECT count() FROM " + database +
                    ".kline_recovery_run WHERE committed FORMAT TSV") ==
          "4");
    CHECK(Query(config.endpoint,
                "SELECT count() FROM (SELECT * FROM " + database +
                    ".kline FINAL) WHERE NOT is_deleted FORMAT TSV") ==
          "1");
    CHECK(Query(config.endpoint,
                "SELECT version, payload.volume, "
...
void TestClickHouseRejectsMismatchedSchema(const std::string& endpoint) {
    const auto unique = std::chrono::system_clock::now()
                            .time_since_epoch().count();
    const std::string database =
        "l2kline_bad_schema_" + std::to_string(unique);
    KLineClickHouseConfig config{};
    ...
    std::string error;
    std::unique_ptr<KLineClickHouseSink> initializer =
        KLineClickHouseSink::Create(config, &error);
    ...
    CHECK(Query(endpoint,
                "ALTER TABLE " + database +
                    ".kline_recovery_run MODIFY COLUMN "
    ...
    CHECK(external != nullptr);
    CHECK(!external->Start(&error));
    CHECK(error.find("KLine recovery-run column contract mismatch") !=
          std::string::npos);
